...
- **mtr capability** (§4.6): `setcap cap_net_raw+ep /usr/bin/mtr-packet` in the `Containerfile` is honoured by rootful Podman / Docker. **Rootless Podman drops the file capability** in the user namespace and `mtr-packet` then fails with `Failure to start mtr-packet: Invalid argument`; the Quadlet must add `AddCapability=CAP_NET_RAW` explicitly. UDP mode does not remove this requirement.
